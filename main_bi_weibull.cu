...
// Normal distribution to bi-Weibull distribution 20250803 Sakai
//           
#include <stdio.h>
#include <math.h>
#include <errno.h>
...
// Sakai 2025/08/03
double bi_weibull_q = 0.34;
double bi_weibull_N1 = 0.82;
double bi_weibull_N2 = 1.04;
double bi_weibull_lambda1 = -1.20;
double bi_weibull_lambda2 = 3.56;
// added
...
double calc_pdf_fraction_bi_weibull(double phi); //Sakai 2025/08/03 
...
void mass_release_calc(int zmax, int phidecimal, double phi, double *h, double *atmP, double *atmT, double *windX, double *windY, double *massreleased){
	...
	pdf_fraction = calc_pdf_fraction_bi_weibull(phi); // Sakai 2025/8/3
...
int init_globals(char *config_file) {
  ...
  while (fgets(line, MAX_LINE, in_config) != NULL) {
    ...
    /*else if (!strncmp(token, "LITHIC_DENSITY", strlen("LITHIC_DENSITY"))) {
	  ...
	  else if (!strncmp(token, "bi_weibull_q", strlen("bi_weibull_q"))) { /* 2025/08/03 added Sakai */
		token = strtok_r(NULL, space, ptr1);
		bi_weibull_q = strtod(token, NULL);
		if(WRITE_CONF) fprintf(stderr, "bi_weibull_q = %.6f\n", bi_weibull_q);
	}
	  else if (!strncmp(token, "bi_weibull_N1", strlen("bi_weibull_N1"))) { /* 2025/08/03 added Sakai */
		token = strtok_r(NULL, space, ptr1);
		bi_weibull_N1 = strtod(token, NULL);
		if(WRITE_CONF) fprintf(stderr, "bi_weibull_N1 = %.6f\n", bi_weibull_N1);
	}
	  else if (!strncmp(token, "bi_weibull_N2", strlen("bi_weibull_N2"))) { /* 2025/08/03 added Sakai */
		token = strtok_r(NULL, space, ptr1);
		bi_weibull_N2 = strtod(token, NULL);
		if(WRITE_CONF) fprintf(stderr, "bi_weibull_N2 = %.6f\n", bi_weibull_N2);
	}
	  else if (!strncmp(token, "bi_weibull_lambda1", strlen("bi_weibull_lambda1"))) { /* 2025/08/03 added Sakai */
		token = strtok_r(NULL, space, ptr1);
		bi_weibull_lambda1 = strtod(token, NULL);
		if(WRITE_CONF) fprintf(stderr, "bi_weibull_lambda1 = %.6f\n", bi_weibull_lambda1);
	}
	  else if (!strncmp(token, "bi_weibull_lambda2", strlen("bi_weibull_lambda2"))) { /* 2025/08/03 added Sakai */
		token = strtok_r(NULL, space, ptr1);
		bi_weibull_lambda2 = strtod(token, NULL);
		if(WRITE_CONF) fprintf(stderr, "bi_weibull_lambda2 = %.6f\n", bi_weibull_lambda2);
	}
    else continue;
  }
...
void obtaintheoreticallyreleased(RELEASE *r){
	...
	for(int phiint = MIN_GRAINSIZE - MAX_GRAINSIZE - 1; phiint >= 0; phiint--){
		for(int phidecimal = 0; phidecimal < PHIDECDIM; phidecimal++){
			phi = phiint + MAX_GRAINSIZE + 1 - phidecimal * INTERVAL_DECIMAL_PHI;
			//pdf_fraction += calc_pdf_fraction(phi);
			pdf_fraction += calc_pdf_fraction_bi_weibull(phi); // Sakai 2025/08/03 added
		}// END OF DECIMAL PHI LOOP
...
// Sakai 2025/08/03 Normal distribution to bi-Weibull distribution
double calc_pdf_fraction_bi_weibull(double phi) {
    double jacobian;
	double term1, term2;
	double pdf_d;
	double pdf_phi;
	double frac;
    double d = pow(2.0, -phi);
	double bi_weibull_lambda1_mm = pow(2.0, -bi_weibull_lambda1);
	double bi_weibull_lambda2_mm = pow(2.0, -bi_weibull_lambda2);

	// Bi-Weibull probability density function (Costa et al., 2016, A.2)
	// f(d) = q * (n1/lambda1) * (d/lambda1)^(n1) * exp[-(d/lambda1)^n1] + 
	//        (1 - q) * (n2/lambda2) * (d/lambda2)^(n2) * exp[-(d/lambda2)^n2]

    // Bi-Weibull probability density function
    term1 = bi_weibull_q * (bi_weibull_N1 / bi_weibull_lambda1_mm) * 
            pow(d / bi_weibull_lambda1_mm, bi_weibull_N1) * 
            exp(-pow(d / bi_weibull_lambda1_mm, bi_weibull_N1));
    
    term2 = (1.0 - bi_weibull_q) * (bi_weibull_N2 / bi_weibull_lambda2_mm) * 
            pow(d / bi_weibull_lambda2_mm, bi_weibull_N2) * 
            exp(-pow(d / bi_weibull_lambda2_mm, bi_weibull_N2));
    
	// Calculate the Jacobian: |dd/d(phi)|
    // d = 2^(-phi)
    // dd/d(phi) = d/d(phi) [e^(-phi * ln(2))]
    //           = e^(-phi * ln(2)) * -ln(2)
    //           = 2^(-phi) * -ln(2)
    // |dd/d(phi)| = |2^(-phi) * -ln(2)| = 2^(-phi) * ln(2)
	
	jacobian = d * log(2.0);
	
	pdf_d = term1 + term2;
	pdf_phi = pdf_d * jacobian;

    frac = pdf_phi * INTERVAL_DECIMAL_PHI;
    return frac;
}
